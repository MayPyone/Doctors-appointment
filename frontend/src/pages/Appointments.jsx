...
function Appointments() {
  ...
  const getAvailableSlots = async () => {
    if (!docInfo || !docInfo.slots_booked) {
      return; // Early return if docInfo or slots_booked is not available
    }
    setDocSlots([])
  ...
  return docInfo && (
    <div>
      <div className='flex flex-col sm:flex-row gap-4'>
        <div>
          <img className='bg-primary w-f sm:max-w-72 rounded-lg' src={docInfo.image} alt="doctor picture" />
        </div>

        <div className='flex-1 border borde-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <div className='flex items-center gap-2 text-sm mt-1 text-gray-600'>
            ...
            <button className='py-0.5 x-2 border text-xs rounded-full'>{docInfo.experience}</button>
          </div>
          <div>
            <p className='flex items-center gap-1 text-sm font-medium text-gray-500 mt-3'>About <img src={assets.info_icon} /></p>
      ...
      <div className='sm:ml-72 sm:pl4 mt-4 font-medium text-gray-700'>
        <p>Booking slots</p>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
          {docSlots.length && docSlots[slotIndex].map((item, index) => (
            <p onClick={() => setSlotTime(item.time)} className={`text-sm font-ligtht flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300'}`} key={index}>
